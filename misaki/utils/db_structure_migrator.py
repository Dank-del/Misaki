...
from misaki import bot, OWNER_ID
from misaki.services.mongo import mongodb
from misaki.utils.logger import log
from misaki.versions import DB_STRUCTURE_VER


async def notify_bot_owner(old_ver, new_ver):
    await bot.send_message(
        OWNER_ID,
        f"Misaki database structure was updated from <code>{old_ver}</code> to <code>{new_ver}</code>"
    )
...
if not (data := mongodb.db_structure.find_one({'db_ver': {"$exists": True}})):
    ...
    if DB_STRUCTURE_VER > curr_ver:
        ...
        while curr_ver < DB_STRUCTURE_VER:
            ...
            log.debug("Importing: misaki.db." + str(new_ver))
            import_module("misaki.db." + str(new_ver))
